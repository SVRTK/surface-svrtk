...
if [ "$#" -ne 8 ]; then
    echo ""
    echo "Draw-EM based fetal brain surface generation from BOUNTI labels"
    echo
    echo "Source code: https://github.com/SVRTK/surface-svrtk"
    echo "Docker: https://hub.docker.com/r/fetalsvrtk/surface"
    echo
    echo "Usage : bash /software/surface-scripts/bounti-surface-generation-v2.sh "
    ...
    echo "[full path to Internal labels file - e.g., /home/data/in-files/sub-1234_ses-000-mask-brain_internal-1.nii.gz]"
...
echo " - Internal labels file : " ${internal_label_in}
...
#parameters_dir=/home/data/surface-scripts
...
if [ ! -f ${workdir}/${segdir}/${subj}-${internal_labels_roi}.nii.gz ];then
    cp ${internal_label_in} ${workdir}/${segdir}/${subj}-${internal_labels_roi}.nii.gz
fi
...
echo " - Processing segmentations (adding CC to BOUNTI) ..."
