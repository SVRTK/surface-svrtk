...
if [ ${completed} -eq 0 ]; then
    ...
    python3 ${mirtk_dir}/recon-neonatal-cortex --config /software/surface-scripts/recon_config-bounti.cfg --sessions=${subj} --prefix=surfaces/${subj}/vtk/${subj} --temp=surfaces/${subj}/vtk/temp-recon/${subj} --white --pial --verbose
...
if  [ ! -f ${outvtk}/${subj}.L.pial.native.surf.vtk ];then
    ...
    echo 
    echo " - merging step did not work ..." ${subj} 
    echo 
