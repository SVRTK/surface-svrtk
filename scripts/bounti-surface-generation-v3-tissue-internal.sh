...
if [ "$#" -ne 7 ]; then
    ...
    echo
    echo "Usage : bash /software/surface-scripts/bounti-surface-generation-v3-tissue-intenral.sh "
...
if [ ! -f ${workdir}/${segdir}/${subj}-${internal_labels_roi}.nii.gz ];then
    ...
    bash /software/surface-scripts/label-propagation-internal-roi.sh ${workdir}/${t2dir}/${subj}.nii.gz ${workdir}/${segdir}/${subj}-${bounti_labels_roi}.nii.gz /${workdir}/${segdir}/${subj}-${internal_labels_roi}.nii.gz > /home/tmp.txt
